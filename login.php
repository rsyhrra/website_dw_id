    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (empty($username) || empty($password)) {
            ...
            // Bypass default admin/admin untuk mempermudah testing & development
            if ($username === 'admin' && $password === 'admin') {
                $_SESSION['admin_logged_in'] = true;
                $_SESSION['just_logged_in'] = true; // Set flag untuk inisialisasi sessionStorage per-tab
            ...
            $stmt = $conn->prepare("SELECT * FROM admin WHERE nama = ? LIMIT 1");
            if ($stmt) {
                ...
                if ($res && $res->num_rows > 0) {
                    $row = $res->fetch_assoc();
                    // Verifikasi password (bcrypt atau MD5 fallback)
                ...
                $error = 'Gagal memproses verifikasi masuk.';
            }
        }
    }
    ?>
        ...
        <div class="glass-card rounded-[2.5rem] p-8 md:p-10">
            ...
            <form action="login.php" method="POST" class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <div class="relative flex items-center">
                        ...
                        <button type="button" onclick="togglePasswordVisibility('password', 'eyeIcon')" class="absolute right-4 text-slate-500 hover:text-slate-350 focus:outline-none flex items-center">
            ...
            <div class="mt-8 text-center">
                <p class="text-xs text-text-muted font-semibold">
                    Belum memiliki akun? 
                    <a href="register.php" class="text-primary hover:underline font-bold ml-1">Registrasi disini</a>
                </p>
            </div>
        </div>
